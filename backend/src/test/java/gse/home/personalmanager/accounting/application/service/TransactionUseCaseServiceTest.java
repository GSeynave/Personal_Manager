...
class TransactionUseCaseServiceTest extends UnitTestBase {
   ...
   * 
   * when(repository.findAllByDateBetween(minDate,
   * maxDate)).thenReturn(transactions);
   * when(transactionService.getTransactionCategoryDetails(transactions)).
   * thenReturn(expected);
   * 
   * // Act
   * List<TransactionSummaryDTO> result =
   * transactionUseCaseService.getAllTransactions(minDate, maxDate);
   * 
   * // Assert
   ...
   * 
   * when(repository.findAllByDateBetween(minDate,
   * maxDate)).thenReturn(emptyList);
   * when(transactionService.getTransactionCategoryDetails(emptyList)).thenReturn(
   * expected);
   * 
   * // Act
   * List<TransactionSummaryDTO> result =
   ...
   * LocalDate maxDate = LocalDate.of(2024, 1, 31);
   * List<Transaction> transactions = List.of(createTransaction(1));
   * AccountingSummaryDTO expected = new AccountingSummaryDTO(2000.0, 500.0,
   * 100.0);
   * 
   * when(repository.findAllByDateBetween(minDate,
   * maxDate)).thenReturn(transactions);
   * when(transactionService.getTransactionSummary(transactions)).thenReturn(
   * expected);
   * 
   * // Act
   * AccountingSummaryDTO result =
   * transactionUseCaseService.getTransactionSummary(minDate, maxDate);
   * 
   * // Assert
   * assertThat(result).isEqualTo(expected);
   * verify(repository).findAllByDateBetween(minDate, maxDate);
   ...
   * createTransaction(2));
   * 
   * when(transactionService.fromCSVRowToTransactionList(csvRows)).thenReturn(
   * transactions);
   ...
   * // Act
   * Integer result =
   * transactionUseCaseService.importCSVRows(Collections.emptyList());
   * 
   * // Assert
   * assertThat(result).isEqualTo(0);
   * verify(repository, never()).save(any(Transaction.class));
   ...
   * List<Transaction> transactions = List.of(createTransaction(1),
   * createTransaction(2));
   * when(repository.findAllByCategory(TransactionCategory.NONE,
   * Pageable.ofSize(10))).thenReturn(new PageImpl<>(transactions,
   * Pageable.ofSize(10), 10));
   * 
   * // Act
   * UncategorizedTransactionDTO result =
   * transactionUseCaseService.getUncategorizedTransactions();
   * 
   * // Assert
   ...
   * @Test
   * void
   * getUncategorizedTransactions_shouldReturnNull_whenNoUncategorizedTransactions
   * () {
   * // Arrange
   * when(repository.findAllByCategory(TransactionCategory.NONE,
   * Pageable.ofSize(10))).thenReturn(Page.empty());
   * 
   * // Act
   * UncategorizedTransactionDTO result =
   * transactionUseCaseService.getUncategorizedTransactions();
   ...
   * @Test
   * void
   * updateTransactionsToCategorize_shouldUpdateSubCategoryAndCustomCategory() {
   * // Arrange
   ...
   * assertThat(transaction.getSubCategory()).isEqualTo(TransactionSubCategory.
   * HOUSE);

...
class TransactionServiceTest extends UnitTestBase {
  ...
  /*
   * private Transaction createTransaction(int id, double amount, TransactionType
   * type, TransactionCategory category) {
   * Transaction transaction = new Transaction();
   * transaction.setId(id);
   * transaction.setAmount(amount);
   * transaction.setImportLabel("Imported label");
   * transaction.setCustomLabel("Test transaction " + id);
   ...
   * @Test
   * void
   * getTransactionCategoryDetails_shouldReturnEmptyList_whenTransactionsAreEmpty(
   * ) {
   * // Arrange
   ...
   * transactionService.getTransactionCategoryDetails(transactions);
   * 
   * // Assert
   ...
   * createTransaction(1, -100.0, TransactionType.DEBIT, "GROSSERY"),
   * createTransaction(2, -200.0, TransactionType.DEBIT, "BILL"),
   * createTransaction(3, -100.0, TransactionType.DEBIT, "GROSSERY"));
   * 
   * // Act
   * List<TransactionSummaryDTO> result =
   * transactionService.getTransactionCategoryDetails(transactions);
   ...
   * .filter(s -> s.getCategory().equals("GROSSERY"))
   * .findFirst()
   * .orElse(null);
   * 
   ...
   * assertThat(grocerySum.getMaxExpected()).isEqualTo(TransactionCategory.
   * GROSSERY.getMaxExpected());
   ...
   * // Arrange
   * List<Transaction> transactions = List.of(
   * createTransaction(1, -100.0, TransactionType.DEBIT,
   * TransactionCategory.GROSSERY),
   * createTransaction(2, -50.0, TransactionType.DEBIT,
   * TransactionCategory.SAVING),
   * createTransaction(3, -30.0, TransactionType.DEBIT,
   ...
   * // Arrange
   * List<Transaction> transactions = List.of(
   * createTransaction(1, 500.0, TransactionType.CREDIT,
   * TransactionCategory.SALARY),
   * createTransaction(2, -100.0, TransactionType.DEBIT,
   * TransactionCategory.GROSSERY));
   * 
   * // Act
   * List<TransactionSummaryDTO> result =
   ...
   * assertThat(salarySum.getExpense()).isEqualTo(0.0); // Credits are not counted
   * as expenses
   * }
   * 
   * @Test
   ...
   * // Arrange
   * List<Transaction> transactions = List.of(
   * createTransaction(1, 2000.0, TransactionType.CREDIT,
   * TransactionCategory.SALARY),
   * createTransaction(2, -500.0, TransactionType.DEBIT,
   * TransactionCategory.BILL),
   * createTransaction(3, -300.0, TransactionType.DEBIT,
   * TransactionCategory.GROSSERY),
   * createTransaction(4, -100.0, TransactionType.DEBIT,
   * TransactionCategory.SAVING));
   * 
   * // Act
   * AccountingSummaryDTO result =
   * transactionService.getTransactionSummary(transactions);
   * 
   * // Assert
   ...
   * // Arrange
   * List<Transaction> transactions = List.of(
   * createTransaction(1, 100.555, TransactionType.CREDIT,
   * TransactionCategory.SALARY));
   * 
   * // Act
   * AccountingSummaryDTO result =
   ...
   * transactionService.fromCSVRowToTransactionList(csvRows);
   * 
   * // Assert
   * assertThat(result).hasSize(2);
   * 
   ...
   * for (TransactionCategory category : List.of(TransactionCategory.BILL,
   * TransactionCategory.GROSSERY,
   * TransactionCategory.CAR)) {
   * transactions.add(createTransaction(transactions.size() + 1, -100.0,
   * TransactionType.DEBIT, category));
   * }
   * 
   * // Act
   * List<TransactionSummaryDTO> result =

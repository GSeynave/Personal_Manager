...
public class WalletService {
  ...
  public Wallet createWallet(String name, String description, AppUser owner, Long tenantId) {
    ...
    var permission = new WalletPermission();
    permission.setUser(owner);
    permission.setRolePermission(WalletRole.OWNER);
    permission.setWallet(wallet);
    wallet.setWalletPermissions(List.of(permission));
   ...
   */
  @Transactional
  public WalletPermission grantPermission(Long walletId, Long userId, WalletRole role, AppUser currentUser,
      AppUser targetUser) {
    Wallet wallet = walletRepository.findById(walletId)
        .orElseThrow(() -> new ResourceNotFoundException("Wallet", walletId));
    ...
    permission.setRolePermission(role);
   ...
   */
  @Transactional
  public WalletPermission updatePermission(Long walletId, Long userId, WalletRole role, AppUser currentUser,
      AppUser targetUser) {
    Wallet wallet = walletRepository.findById(walletId)
        .orElseThrow(() -> new ResourceNotFoundException("Wallet", walletId));
  ...
  public boolean hasWriteAccess(Wallet wallet, AppUser user) {
    ...
    return permissionRepository.findByWalletAndUser(wallet, user)
        .map(permission -> WalletRole.WRITE.equals(permission.getRolePermission()))
        .orElse(false);
  }
   ...
   * The balance is retrieved from the most recent transaction's currentBalance
   * field.
